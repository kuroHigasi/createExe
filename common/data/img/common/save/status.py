...
class Status:
    def nextStatus(statusForm, opeForm, saveForm):
        ...
        if (cmn.Judge.click(buckX, buckY, sizeX, sizeY, x, y, clickX, clickY, opeForm.isLeftClick())):
            nextStatus = saveForm.PRE_STATUS()
        if not (homeX == -1 and homeY == -1):
            if (cmn.Judge.click(homeX, homeY, homeSizeX, homeSizeY, x, y, clickX, clickY, opeForm.isLeftClick())):
